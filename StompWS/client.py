...
class StompWS:
    ...
    def __init__(self, url, headers=None, reconnect=10):
    ...
    def _connect(self, timeout=0):
        # self.ws.run_forever(ping_interval=self.ping_interval, ping_timeout=self.ping_timeout, ping_payload=Byte['LF']
        #                     , reconnect=10)
        ...
        while self.opened is False:
            ...
            if 0 < timeout < total_ms:
                raise TimeoutError("Connection to {} timed out".format(self.url))
    ...
    def _on_message(self, ws_app, message, *args):
        ...
        if frame.command == "CONNECTED":
            ...
            if frame.headers['heart-beat'] is not None:
                self.ping_interval, self.ping_timeout = frame.headers['heart-beat'].split(',')
                self.ping_interval = int(self.ping_interval)
                self.ping_timeout = int(self.ping_timeout)
